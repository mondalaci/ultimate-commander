	     ...
	     const char *video_path, int size)
{
	...
	if (gdk_pixbuf_save (with_holes, path, "png", &err, NULL) == FALSE)
	{
		if (err != NULL)
		{
			g_print ("depot-video-thumbnailer couln't write the thumbnail '%s' for video '%s': %s\n", path, video_path, err->message);
			g_error_free (err);
		} else {
			g_print ("depot-video-thumbnailer couln't write the thumbnail '%s' for video '%s'\n", path, video_path);
		}
...
time_monitor (gpointer data)
{
	...
	g_print ("depot-video-thumbnailer couln't thumbnail file: '%s'\n"
...
int main (int argc, char *argv[])
{
	...
	GdkPixbuf *pixbuf;
	int i, length, size, thumbs_num;
	char *number_of_thumbs, *input, *output;
	char outputfilename[512];
	char **endptr;
	
	if (argc <= 2 || strcmp (argv[1], "-h") == 0 ||
	    strcmp (argv[1], "--help") == 0) {
		g_print ("Usage: %s [-s <size>] <number-of-thumbnails> <input> <output-prefix> [xine backend options]\n", argv[0]);
		return -1;
	}
	...
	if (!strcmp (argv[1], "-s")) {
		...
		if (argc != 6)
		{
		    g_print("An incorrect number of arguments has been given.\n");
		    exit(1);
		}
	} else {
		number_of_thumbs = argv[1];
		input = argv[2];
		output = argv[3];
		size = 128;
	...
	thumbs_num = strtol(number_of_thumbs, endptr, 10);
	
	if (endptr != NULL)
	{
		g_print ("depot-video-thumbnailer: invalid number-of-thumbnails argument given.\n");
		exit (1);
	}
	...
	if (err != NULL)
	{
		g_print ("depot-video-thumbnailer couln't create the video "
				"widget.\nReason: %s.\n", err->message);
		g_error_free (err);
		exit (1);
	}
	...
	if (bacon_video_widget_open (bvw, input, &err) == FALSE)
	{
		if (err->code == BVW_ERROR_STILL_IMAGE)
		{
			...
			g_print ("depot-video-thumbnailer couln't open file '%s'\n"
					"Reason: %s.\n", input, err->message);
			g_error_free (err);
			exit (1);
		}
	...
	if (err != NULL)
	{
		g_print ("depot-video-thumbnailer couln't play file: '%s'\n"
				"Reason: %s.\n",
				input, err->message);
		g_error_free (err);
		exit (1);
	...
	{
		g_print ("depot-video-thumbnailer: '%s' isn't thumbnailable\n"
				"Reason: %s\n",
	...
	for (i=1; i<=thumbs_num; i++)
	{
	    float position = (float)rand() / RAND_MAX;
	    sprintf(outputfilename, "%s%i.png", output, i);
	    g_print("Capturing image %i of %i as %s... \n", i, thumbs_num, outputfilename);

	    if (bacon_video_widget_seek
			(bvw, ((position)), NULL) == FALSE)
	    {
	    ...
	    if (pixbuf == NULL)
	    {
	        g_print ("depot-video-thumbnailer couln't get a picture from '%s'\n", input);
	        exit (1);
	    }

	    save_pixbuf (pixbuf, outputfilename, input, size);
	    gdk_pixbuf_unref (pixbuf);
	}
